...
En el training set, tenemos algunos pacientes con cuatro chequeos y el resto solamente tiene tres chequeos. Para esos con sólo tres mediciones, la cuarta medición la podemos encontrar en el test set. Entonces, al final, podemos tener cuatro filas por cada paciente. Lo que hice fue concatenar todos estos chequeos para los mismos pacientes en una sola fila.
...
Por ejemplo, dado un paciente, puedo concatenar los cuatro chequeos en una sola fila. La clara ventaja de esto es que, en cada fila, tengo mucha más información sobre el paciente: todas las HAZ. WAZs, BMIZs, etc en una sola fila. Por otro lado, esto significa que ahora tengo un train set mucho más chico, **6200**~ contra el original de **42000**~.

Otra cosa que puedo hacer es, en lugar de concatenar los cuatro chequeos, concateno solo tres por fila como en la imagen.
Esto implica que ahora puedo usar los checkeos 1, 2, 3  y los 2, 3, 4 de cada paciente y hacer dos filas diferentes. Con este enfoque tengo el doble de filas de entrenamiento, **12000**~.
